...
/* ----------------------------------------------------------------------
   Contributing authors 2019:
     Christian Seidl, Johannes Hoermann (IMTEK, University of Freiburg)
------------------------------------------------------------------------- */

#include "pair_lj_charmmfsw.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "atom.h"
...
PairLJCharmmfsw::PairLJCharmmfsw(LAMMPS *lmp) :
  Pair(lmp)
{
  implicit = 0;
  mix_flag = ARITHMETIC;
  ...
  dihedflag = 0; // jlh: double-check meaning of flag
...
PairLJCharmmfsw::~PairLJCharmmfsw()
{
...
void PairLJCharmmfsw::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,evdwl12,evdwl6,ecoul,fpair;
  double r,rinv,r3inv,rsq,r2inv,r6inv,forcelj,factor_coul,factor_lj;
  double switch1;
...
void PairLJCharmmfsw::settings(int narg, char **arg)
{
  if (narg != 2 )
...
void PairLJCharmmfsw::coeff(int narg, char **arg)
{
  if (narg != 4 && narg != 6)
...
void PairLJCharmmfsw::init_style()
{
...
double PairLJCharmmfsw::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCharmmfsw::write_data(FILE *fp)
{
...
void PairLJCharmmfsw::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void PairLJCharmmfsw::write_restart(FILE *fp)
{
...
void PairLJCharmmfsw::read_restart(FILE *fp)
{
...
void PairLJCharmmfsw::write_restart_settings(FILE *fp)
{
...
void PairLJCharmmfsw::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairLJCharmmfsw::
single(int i, int j, int itype, int jtype,
       double rsq, double factor_coul, double factor_lj, double &fforce)
{
  double r,rinv,r2inv,r3inv,r6inv,forcelj;
  double philj,philj12,philj6;
  double switch1;
...
void *PairLJCharmmfsw::extract(const char *str, int &dim)
{
  dim = 2;
  ...
  // Taking out this line will result in incompatibility with CHARMM dihedral styles:
