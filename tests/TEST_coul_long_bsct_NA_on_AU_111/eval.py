...
def loadlog(fn):
    ...
    while len(l.split()) == 0 or l.split()[0] != 'Loop':
        ...
        if len(l.split()) > 0:
            ...
            #    l = f.readline()
            if l.split()[0] == 'ERROR':
                if debug: sys.stderr.write("Encountered error: {}".format(l))
                break
            elif not l.split()[0].isdigit():
                if debug: sys.stderr.write("Ignoring line: {}".format(l))
            else:
                if debug: sys.stderr.write("Read line: {}".format(l))
                s.write(l)
        l = f.readline()
    return s
...
def eval(    
    ...
    **kwargs ):    
    ...
    s = loadlog(log_file)
    if debug:
        sys.stderr.write( "Read following content:\n" ) 
